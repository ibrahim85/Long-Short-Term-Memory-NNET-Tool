...
def load_normalize_and_generate_test_and_training_data():
    ...
    dataframe = None
    try:
        dataframe = pandas.read_csv(parameters["input_filename"], usecold=[1], engine='python', skipfooter=3)
    except Exception, e:
        raise Exception("Error in reading %s: %s", input_filename, e)
    ...
    training_data, test_data = dataset[0:train_size,:], dataset[train_size:len(dataset),:]
...
def plot_and_save(dataset, train_data_prediction, test_data_prediction, scaler, look_back=1):
    ...
    plt.plt(test_prediction_plot_data)
    plt.savefig(configuration["output_filename"]+".png")

'''
Run the neural network

Takes the parameters present in the configuration
...
def run():
    ...
    # adding layers
    try:
        model = add_layers(configuration["layer_dimensions"], model)
